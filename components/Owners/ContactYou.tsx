...
export const ContactYou = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async(data) => {
   try {
    const response = await axios.post('https://dygav-suite-default-rtdb.firebaseio.com/contact.json', data)
    await response.data
      ...
      <div className='w-[350px] md:w-[450px] lg:w-full md:px-4 self-center justify-self-center pt-5 md:text-sm text-black900 border-t-4 border-t-p600'>
        ...
        >
          ...
          <label className='relative'>
            ...
            <input
              type='text'
              {...register('fullname', {required:"Nombre es requerido", minLength: {value: 6, message: "El nombre debe tener minimo 6 caracteres"}, maxLength: {value: 60, message: "El nombre debe tener maximo 60 caracteres"} })}
          ...
          <label className='relative'>
            ...
            <input
              type='number'
              {...register('cellphone', {required:"Telefono es requerido", pattern: {value: /^[0-9]+$/i, message: "El formato del telefono es incorrecto"} })}
